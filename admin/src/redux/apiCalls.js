...
import {
  getOrderStart, getOrderSuccess, getOrderFailure, updateOrderStart, updateOrderSuccess, updateOrderFailure, updatePaidStart, updatePaidSuccess, updatePaidFailure, updateDlvrStart, updateDlvrSuccess, updateDlvrFailure
} from "./orderRedux"
...
export const updatePaid = async (id, kok, dispatch) => {
  dispatch(updatePaidStart());
  try {           
    let kokx = "Paid"
    if (kok === "Paid") {
      kokx = "Not Paid" 
    } else { kokx = "Paid" }
    const res = await userRequest.put(`/orders/${id}`, {_id: id, onStatus: kokx});
    dispatch(updatePaidSuccess(id, kokx));
    window.location.reload(false)
  } catch (err) {
    dispatch(updatePaidFailure());
  }
};
